jQuery(function ($) {
	...
	// Add Dependancies
	var head = $("head");
	var elevateZoom = $('<script type="text/javascript" language="Javascript" src="js/jquery.elevatezoom.js"></script>');
	head.add(elevateZoom).appendTo(head);
	*/
	/*
	var elevateZoom = $('<script type="text/javascript" language="Javascript" src="js/jquery.elevatezoom.js"></script>');
	var timelineScript = $('<script type="text/javascript" language="Javascript" src="js/timeline.js"></script>');
	timelineScript.before(elevateZoom);
	*/
	...
	// Create and add the Spine (Centre Line of the Timeline)
...
Twitter was created in March 2006 by Jack Dorsey, Evan Williams, Biz Stone and Noah Glass and by July 2006 the site was launched. The service rapidly gained worldwide popularity, with 500 million registered users in 2012, who posted 340 million tweets per day. The service also handled 1.6 billion search queries per day. In 2013 Twitter was one of the ten most-visited websites, and has been described as "the SMS of the Internet.".</p></div>'
		...
		timelineEvent.hover(
			function () {
				...
				eventImage.click(function () {
					...
					var image = $("<img src='"+imageSource+"' data-zoom-image='"+imageSource+"' />");
					...
					// Remove the box so it is not on top of the image viewer
					box.remove();
					...
					var wPercentage = imageWidth/containerWidth;
					var hPercentage = imageHeight/containerHeight;
					
					console.log("Width"+wPercentage+", Height"+hPercentage);
					
					if (wPercentage > 0.9 || hPercentage > 0.9) {
						image.elevateZoom();
					}
	...
	alert("Click on an image to enlarge it, if the image is still too big, perspective zoom will be enabled. (I know it works on the Jordan = [[Developer alloc] init];).");
})
